...
class Service extends Base {
	...
	useModel(dataset) {
		if (this.Model && dataset instanceof this.Model) {
			let newDataset = {};
	...
	useModelArrayLike(datasets) {
		if (this.Model && isArrayLike(datasets)) {
			datasets = Array.from(datasets).map(dataset => this.useModel(dataset));
		}
	...
	toArray(scope = this) {
		...
		return this.useModelArrayLike(arr);
	}
